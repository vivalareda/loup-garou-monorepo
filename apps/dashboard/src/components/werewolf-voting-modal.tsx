...
export function WerewolfVotingModal({
  ...
  useEffect(() => {
    ...
    socket.on('werewolf:current-votes', handleVotesUpdate);
    socket.on('werewolf:voting-complete', handleVotingComplete);

    return () => {
      socket.off('werewolf:current-votes', handleVotesUpdate);
      socket.off('werewolf:voting-complete', handleVotingComplete);
    };
  }, [socket, isOpen]);
  ...
  const sendVote = useCallback(
    (targetPlayerSocketId: string) => {
      ...
      setPlayerVote(targetPlayerSocketId);
      socket.emit('werewolf:player-voted', targetPlayerSocketId);
    },
    [socket, updateVote, playerVote]
  );
        ...
        <div className="mx-4 w-full max-w-md rounded-lg bg-white p-6">
          <div className="text-center">
            <p className="text-red-600">
              No active player selected or player not connected
            </p>
            <button
