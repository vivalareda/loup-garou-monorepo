...
describe('Full Game Integration - Complete Night Phase', () => {
  it.effect(
    'should complete full night phase with werewolf kill',
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* segmentManager.getCurrentSegmentType).toBe('WITCH-POISON');
        yield* segmentExecution.playSegment;
...
describe('Full Game Integration - Complete Day Phase', () => {
  it.effect(
    'should complete full day phase with voting and elimination',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
  ...
  it.effect(
    'should handle day phase with tie and no elimination',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
        ...
        for (const werewolf of yield* game.getWerewolfList) {
          yield* events.handleWerewolfVote(werewolf.socketId, nightTarget.socketId);
        }
        yield* segmentExecution.finishSegment;
        yield* segmentExecution.finishSegment;
        yield* segmentExecution.playSegment;
        ...
        for (let i = 4; i < alivePlayers.length; i++) {
          const target = i % 2 === 0 ? alivePlayers[1].socketId : alivePlayers[3].socketId;
...
describe('Full Game Integration - Multiple Cycles', () => {
  it.effect(
    'should complete multiple night/day cycles',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
...
describe('Full Game Integration - Winner Detection', () => {
  it.effect(
    'should detect villagers win when all werewolves eliminated',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
  ...
  it.effect(
    'should detect werewolves win when equal to villagers',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
...
describe('Full Game Integration - Lovers Suicide', () => {
  it.effect(
    'should trigger lover suicide when one lover dies',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
        ...
        expect(
          deaths.some((d) => d.playerId === lovers[0].socketId)
        ).toBe(true);
        expect(
          deaths.some((d) => d.playerId === lovers[1].socketId)
        ).toBe(true);
...
describe('Full Game Integration - Hunter Revenge', () => {
  it.effect(
    'should handle hunter revenge when hunter dies',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        ...
        const result = yield* Effect.either(gameActions.handleHunterPlayerPick(target.socketId));
  ...
  it.effect(
    'should handle werewolf kills lover who is hunter',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
        ...
        if (werewolves.length === 0 || hunters.length === 0 || villagers.length === 0) {
          return;
        }
        ...
        expect(deaths.some((d) => d.playerId === hunter.socketId && d.cause === 'WEREWOLVES')).toBe(true);
        expect(deaths.some((d) => d.playerId === lover.socketId && d.cause === 'PARTNER_SUICIDE')).toBe(true);
  ...
  it.effect(
    'should handle lover dies and partner is hunter scenario',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
        ...
        expect(deaths.some((d) => d.playerId === lover.socketId && d.cause === 'WEREWOLVES')).toBe(true);
        expect(deaths.some((d) => d.playerId === hunter.socketId && d.cause === 'PARTNER_SUICIDE')).toBe(true);
...
describe('Full Game Integration - Edge Cases', () => {
  it.effect(
    'should handle all players voting in day phase',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
  ...
  it.effect(
    'should handle witch with no potions',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const events = yield* EventsActions;
  ...
  it.effect(
    'should handle game with minimum players',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
        const segmentExecution = yield* SegmentExecution;
  ...
  it.effect(
    'should handle empty death queue correctly',
    () =>
      Effect.gen(function* () {
        const game = yield* Game;
